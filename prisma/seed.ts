...
async function main() {
  ...
  const hashedCreatorPassword = await bcrypt.hash("creator123", 10);
  ...
  for (let idx = 0; idx < courseSpecs.length; idx++) {
    ...
    const course = await prisma.course.create({
      data: {
        ...
        level: spec.level as "BEGINNER" | "INTERMEDIATE" | "ADVANCED",
        language: "English",
        isPublished: true,
        isFreePreview: true,
  ...
  for (let sIdx = 0; sIdx < students.length; sIdx++) {
    ...
    const studentCoursePool = [...courses].sort(
      (a, b) => (a.id.charCodeAt(0) + sIdx) - (b.id.charCodeAt(0) + sIdx)
    );
  ...
  for (let sIdx = 0; sIdx < students.length; sIdx++) {
    ...
    const cartEligible = courses.filter(
      (c) => !enrolledIds.has(c.id) && !wishlistIds.has(c.id)
    );
  ...
  const counts = {
    ...
    creators: await prisma.user.count({ where: { role: "CREATOR" } }),
